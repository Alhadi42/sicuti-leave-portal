...
const EmployeeList = ({ employees, isLoading, onActionClick, isSearchActive }) => {
  if (isLoading) {
    return (
      <div className="text-center py-8 text-slate-300">
        {isSearchActive ? "Mencari data pegawai..." : "Memuat data pegawai..."}
      </div>
    );
  }
          ...
          <CardContent>
            <div className="text-center py-8">
              <p className="text-slate-400">
                {isSearchActive ? "Tidak ada data pegawai yang cocok dengan pencarian Anda." : "Tidak ada data pegawai untuk ditampilkan."}
              </p>
            </div>
          </CardContent>
        </Card>
    ...
    >
      <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50">
        ...
        <CardContent>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-left py-3 px-4 text-slate-300 font-medium">Nama Jabatan</th>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center space-x-1">
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => onActionClick("Edit Pegawai")}
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => onActionClick("Hapus Pegawai")}
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => onActionClick("Menu Lainnya")}
                          className="text-slate-400 hover:text-white w-8 h-8"
                        >
          ...
          {!isSearchActive && employees.length > 0 && (
             <p className="text-xs text-slate-400 mt-4 px-4">Menampilkan {employees.length} dari potensi lebih banyak data. Gunakan pencarian untuk hasil spesifik.</p>
          )}
        </CardContent>
      </Card>
    </motion.div>
